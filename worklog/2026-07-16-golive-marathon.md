...
- **Post-payment redirect (membership):** after paying the membership Buy Button, the caller is **left on Stripe's default confirmation page** — no route back. The Buy Button's after-payment behavior is set in the **Stripe dashboard** (not our code): Stripe → Buy Button → *After payment → Redirect* → `https://velvetrope2you.com/72-app-dashboard.html`. (Manual dashboard step — still open.)
- ✅ **Worker dead-redirect FIXED (2026-07-20):** the worker sent users to `/dashboard.html` in **three** places — membership `success_url` **and** the Connect `refresh_url`/`return_url` — but the file is `72-app-dashboard.html`, so every one 404'd. This would have broken the call-flow test: a creator finishing Stripe onboarding hit a 404 instead of their dashboard. All three corrected to `/72-app-dashboard.html` (the dashboard already reads `?onboard=complete` / `?signup=success` / `?userId`). Worker redeploys via Workers Builds on push.
